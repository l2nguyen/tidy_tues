...
earnings_temp <- earnings_female %>%
  mutate(group_temp = group) %>%
  filter(group != "Total, 16 years and older") %>%
  mutate(proportion = percent / 100)

ggplot(data = earnings_temp, aes(x = Year, y = proportion)) +
  geom_line(data = earnings_temp %>% select(-group),
            aes(group = group_temp), color = "grey75", size = 0.5, alpha = 0.5) +
  geom_line(aes(color = group), color = "#5dbfb8", size = 1) +
  labs(y = "Women's wages as percent of men's wages",
       x = "Year",
